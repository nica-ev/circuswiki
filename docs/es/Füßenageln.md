---
lang: es
translation_id: fu-enageln
publish: true
tags:
  - spiele
created: 2025-01-25 01:57:09
update: 2026-06-07 01:14:58
title: Clavar pies
description: Un juego de movimiento en el que todos intentan clavar los pies de otras personas mientras permanecen móviles.
authors:
  - Marc Bielert
category:
  - bewegung
Schwierigkeit: fácil
Material: ninguno
Spieldauer: 5-10
source: Tasifan Spielebuch
group-min: 5
group-max: 30
translation_status: machine-translated
translation_source_lang: de
translation_source: docs/de/Füßenageln.md
translation_source_hash: e4f89679f44090b3b2c1e977779d02233459dfa5af50237e46ef713d9f86363c
translation_model: google/gemini-2.5-flash-lite
translation_updated: 2026-06-09T01:39:54+00:00
translation_source_body_hash: e4f89679f44090b3b2c1e977779d02233459dfa5af50237e46ef713d9f86363c
translation_source_metadata_hash: 8af17027beac320f4e0d9d1f66de0227a3210705e81ed622bb4dccc1abe1826f
translation_metadata_model: google/gemini-2.5-flash-lite
translation_metadata_status: machine-translated
translation_metadata_updated: 2026-06-14T15:50:34+00:00
translation_source_localized_metadata_hash: 8af17027beac320f4e0d9d1f66de0227a3210705e81ed622bb4dccc1abe1826f
translation_source_structural_metadata_hash: 14070c6c5efe3fd7a5c25ecc84314673ce4917de9bed2c0bd78e1ef8443fed5c
---
> [!info] Breve descripción
> Un juego de movimiento en el que todos intentan "clavar" los pies de otras personas y permanecer móviles ellos mismos.

**Tamaño del grupo**: a partir de 5 personas
**Dificultad**: fácil
**Material**: ninguno
**Duración del juego**: 5-10 minutos

## **Descripción del juego**:

Cada jugador intenta tocar los pies de los demás jugadores con sus manos. Por lo tanto, la regla es: todos contra todos.

Si un jugador logra tocar el pie de otro, ese pie queda "clavado" en el suelo. Esto significa que ese pie ya no se puede mover de su sitio. Sin embargo, con el otro pie, el que está libre, se puede intentar esquivar. Además, se pueden tocar otros pies en cualquier momento.
Condición básica para este juego: ¡todos los jugadores libres deben permanecer en movimiento!

El juego termina cuando nadie o solo una persona tiene dos pies libres.

## **Fuente**:

Tasifan Spielebuch
