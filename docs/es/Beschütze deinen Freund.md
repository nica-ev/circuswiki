---
lang: es
translation_id: beschutze-deinen-freund
created: 2025-03-17 00:59:34
update: 2026-06-07 00:03:04
publish: true
tags:
  - spiele
title: Protege a tu amigo
description: Un juego de círculo donde las personas se protegen del perseguidor en el centro formando parejas.
authors:
  - Marc Bielert
category:
  - action
  - kreisspiel
Schwierigkeit: fácil
Material: ninguno
Spieldauer: 4-9
source: Tasifan Spielebuch
group-min: 8
group-max: 30
translation_status: machine-translated
translation_source_lang: de
translation_source: docs/de/Beschütze deinen Freund.md
translation_source_hash: 904bdf221f6176b5e39d9891167ddae6244b20ceb3e1399373ea7df3bbc7a714
translation_model: google/gemini-2.5-flash-lite
translation_updated: 2026-06-09T01:24:31+00:00
translation_source_body_hash: 904bdf221f6176b5e39d9891167ddae6244b20ceb3e1399373ea7df3bbc7a714
translation_source_metadata_hash: 48ec91c5fd2961f98a5f1db412467940fefef881b4d35acd094fb7d0a2c58ef4
translation_metadata_model: google/gemini-2.5-flash-lite
translation_metadata_status: machine-translated
translation_metadata_updated: 2026-06-14T15:39:57+00:00
translation_source_localized_metadata_hash: 48ec91c5fd2961f98a5f1db412467940fefef881b4d35acd094fb7d0a2c58ef4
translation_source_structural_metadata_hash: 70a64b521164da3d8d5cbed9e21ad1778d2e3ee4db8c49aa86be92a423b9e268
---
> [!info] Breve descripción
> Un juego de círculo en el que las personas se protegen de quien la liga uniéndose en parejas.

**Tamaño del grupo**: a partir de 8 personas
**Dificultad**: fácil
**Material**: ninguno
**Duración del juego**: 4-9 minutos

## **Descripción del juego**:

Todos los jugadores se colocan en círculo. Se designa a una persona como "ligador" (quien la liga) y se coloca en el centro del círculo. El ligador puede intentar tocar a los jugadores que están en el círculo. Sin embargo, estos pueden protegerse formando parejas. Si se juntan tres personas, el ligador puede elegir a una de ellas y tocarla. Si dos personas se juntan y el ligador no puede tocarlas, puede girar una vez en el círculo. En ese momento, la pareja debe separarse y los dos jugadores deben formar rápidamente pareja con otro vecino. De lo contrario, pueden ser tocados. Si alguien es tocado, esa persona debe ir al centro del círculo y el ligador puede ocupar su lugar.
En grupos más grandes, varios jugadores pueden estar en el centro del círculo e intentar tocar a los jugadores exteriores.

## **Fuente**:

Tasifan Spielebuch
