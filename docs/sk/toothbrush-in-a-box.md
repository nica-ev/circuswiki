---
lang: sk
translation_id: jugglingworld-toothbrush-in-a-box
publish: true
tags:
  - spiele
  - convention-games
  - toothbrush
  - target
  - diabolo
created: 2026-06-12 00:00:00
update: 2026-06-12 22:09:42
title: Zubná kefka v krabici
description: Hráči sa najprv naučia točiť zubnou kefkou a potom sa ju pokúsia hodiť do krabice uprostred miestnosti.
authors:
  - CircusWiki contributors
category:
  - convention-games
  - miscellaneous
  - target
  - diabolo
Schwierigkeit: ľahká
Material: Zubné kefky, krabica alebo kôš
Spieldauer: "5-15"
group-min: 2
group-max: 40
source: "JugglingWorld juggling games page; rewritten for CircusWiki"
translation_status: machine-translated
translation_source_lang: en
source_url: "https://www.jugglingworld.biz/tricks/juggling-games/"
source_section: "Miscellaneous!"
source_permission: "public-place image reviewed as reusable by project maintainer"
translation_source: docs/en/toothbrush-in-a-box.md
translation_source_body_hash: b5edb11f3cf7887a2aadfa42b78d16575106420aa5da9d723dcb17c39b7e3f6e
translation_source_hash: b5edb11f3cf7887a2aadfa42b78d16575106420aa5da9d723dcb17c39b7e3f6e
translation_model: google/gemini-2.5-flash-lite
translation_updated: 2026-06-12T21:35:31+00:00
translation_source_metadata_hash: 9bfeb4842b9e015a6b1f26928f9976aea31a8e5ffd0bb09fedb5b3f8ec0f3777
translation_metadata_model: google/gemini-2.5-flash-lite
translation_metadata_status: machine-translated
translation_metadata_updated: 2026-06-14T16:04:04+00:00
translation_source_localized_metadata_hash: 9bfeb4842b9e015a6b1f26928f9976aea31a8e5ffd0bb09fedb5b3f8ec0f3777
translation_source_structural_metadata_hash: 4793a8c677d72b40d512ddf5d5bfa03ed974c381f033d9eb678d42d82f80ea72
---
> [!info] Krátky opis
> Hráči sa najprv naučia točiť zubnou kefkou a potom sa ju pokúsia hodiť do škatule uprostred miestnosti.

**Veľkosť skupiny**: 2 až 40 hráčov
**Obtiažnosť**: jednoduchá
**Materiál**: Zubné kefky, škatuľa alebo kôš
**Trvanie**: približne 5 – 15 minút

## Opis hry

Hráči sa najprv naučia točiť zubnou kefkou a potom sa ju pokúsia hodiť do škatule uprostred miestnosti. Hra funguje ako hod na cieľ po krátkej fáze učenia sa zručnosti.

## Variácie

V neskorších kolách posuňte škatuľu ďalej.

## Bezpečnostné pokyny

Udržujte hrací priestor voľný a pred začiatkom kola vymedzte jeho hranice. V prípade kontaktných hier zamerajte pozornosť na rekvizity, nie na telá. Pri hrách s hádzaním, jazdou, balancovaním alebo akrobaciou nechajte dostatok priestoru na neúspešné pokusy a kolo ukončite, ak skupina začne podstupovať nebezpečné riziká.

## Zdroj

[JugglingWorld - Juggling Games](https://www.jugglingworld.biz/tricks/juggling-games/), sekcia: Miscellaneous!.
